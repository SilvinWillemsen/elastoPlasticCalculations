...
\fbox{\parbox{0.93\linewidth}{
 ...
 $i = 0$\\
  \While{$\epsilon <$ tol}{
    ...
    3. $a^n$\\
    4. $\dot{z}(v^n_{(i)}, z^n_{(i)})$\\
    5. $g_1$, $g_2$\\
    6. Calculate $\epsilon$: $\epsilon = \Big|\dot{z}(v^n_{(i)}, z^n_{(i)}) - \dot{z}(v^n_{(i-1)}, z^n_{(i-1)})\Big|$\\
    7.--11. Compute derivatives of 1.--5. in the same order. \\
    12. Perform vector NR to obtain $v_{(i+1)}$ and $z_{(i+1)}$\\
    13. Increment $i$: $i = i + 1$
  }
